...
void control_s(){
	...
	printf("regdest %d regwrite %d alusrc %d memwrite %d memread %d memtoreg %d pcsrc2 %d pcsrc1 %d\n", regdest,regwrite,alusrc,memwrite,memread,memtoreg, pcsrc2,pcsrc1);
}
...
void main(){
	...
	while(1){
		printf("NNNNNNNNNNNNNNNNN cycle : %d NNNNNNNNNNNNNNNNNN\n",cycle);
		fetch();
		decode();
		control_s();
		execution();	
...
//0		printf("the fucking result is : r[2]=%d\n\n\n\n \n",r[2]);
		regdest=0,regwrite=0,alusrc=0,memwrite=0,memread=0,memtoreg=0,pcsrc2=0,pcsrc1=0,funct=0,opcode=0;
